...
The following table depicts a shortened version of the architecture of the \textit{ReviewMap}.
\begin{table}[tb]
\centering
...
\end{tabular}
\end{table}\\
At this point, \toolname\ has at disposal two different sources information, \ie the set of \textit{\Crash-objects }collected and classified inside the bucket constructed in the clustering phase and a set of preprocessed and categorized \textit{user reviews}. \\
...
This method preprocesses the reports expoiting the information retrivial techniques used by \textbf{INFUSA-TA}, explained at the beginning of the section \ref{tool: linking}.
...
will be
\begin{center}
\emph{BulkCursorToCursorAdaptor}
\end{center} 
Indeed, the regular expression firstly splits the line by a \textit{round bracket} creating a local static array of Strings. Afterwards, it selects the second position in this array. So, it splits the just created array by a \textit{period}, forming a second array of Strings. Finally, it selects the first position of this latter. 
...
\item \texttt{line.split({\color{blue}"\textbackslash\textbackslash("})[1]} \\
\emph{BulkCursorToCursorAdaptor.java:64)}

\item \texttt{line.split({\color{blue}"\textbackslash\textbackslash("})[1].split({\color{blue}"\textbackslash\textbackslash."})} \\
\emph{BulkCursorToCursorAdaptor, \\
java:64)}

\item \texttt{line.split({\color{blue}"\textbackslash\textbackslash("})[1].split({\color{blue}"\textbackslash\textbackslash."})[0]} \\
\emph{BulkCursorToCursorAdaptor}

\end{itemize}
Once all classes present in the trace stack have been extracted, \toolname\ controls whether in the list of classes coming from the source code there are elements in common with those coming from the stack traces. 
In the event that there are some classes in common, all additional words, \ie the methods found in that class, are added to the above illustrated \textit{firstJavaStackTraceLine}. 
...
public List<String> getAdditionalWordsFromSourceCode(){
		HashMap<String, String> classes = Augmenter.getClassesFromCSV();
		List<String> augmentedStackTrace = convertToList(this.firstJavaStackTraceLine);
		classes.forEach((pClass, additionalWords)-> {
			if (this.stackTraceMethods.contains(pClass)) {
...
\end{lstlisting}
First of all, the words coming from the source code are stored inside a local hash map called \textit{classes}.
This hash map has as keys a set of Strings, which represent the name of the classes found while parsing the source code. 
Their values consists of a set of Strings, each of them contains a list of preprocessed methods split by a white space.  
\textit{Line 7} goes through each key element in the hash map and whether \toolname\ notices that there is an element in common between the stack trace classes and those from the source code (\textit{line 8}), it adds its additional words to the list representing the \textit{augmentedStackTrace} (\textit{line 9}).
At the end of this process, presented in the first step of the augmenting process looks like the following: 
\begin{center}
\smallbreak
